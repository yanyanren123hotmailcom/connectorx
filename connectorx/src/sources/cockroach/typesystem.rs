...
impl<'a> From<&'a Type> for CockroachTypeSystem {
    fn from(ty: &'a Type) -> CockroachTypeSystem {
        use CockroachTypeSystem::*;
        match ty.name() {
            "int2" => Int2(true),
            "int4" => Int4(true),
            "int8" => Int8(true),
...
pub struct CockroachTypePairs<'a>(pub &'a Type, pub &'a CockroachTypeSystem);

// Link (postgres::Type, connectorx::PostgresTypes) back to the one defiend by the postgres crate.
impl<'a> From<CockroachTypePairs<'a>> for Type {
    fn from(ty: CockroachTypePairs) -> Type {
        use CockroachTypeSystem::*;
        match ty.1 {
            Enum(_) => Type::TEXT,
