//! Transport from Cockroach Source to Arrow Destination.
...
#[derive(Error, Debug)]
pub enum CockroachArrowTransportError {
    #[error(transparent)]
...
/// Convert Cockroach data types to Arrow data types.
pub struct CockroachArrowTransport<P, C>(PhantomData<P>, PhantomData<C>);

macro_rules! impl_cockroach_transport {
    ($proto:ty, $tls:ty) => {
        impl_transport!(
            name = CockroachArrowTransport<$proto, $tls>,
            error = CockroachArrowTransportError,
            systems = CockroachTypeSystem => ArrowTypeSystem,
            route = CockroachSource<$proto, $tls> => ArrowDestination,
            mappings = {
...
impl<P, C> TypeConversion<NaiveTime, NaiveTimeWrapperMicro> for CockroachArrowTransport<P, C> {
...
for CockroachArrowTransport<P, C>
{
...
impl<P, C> TypeConversion<DateTime<Utc>, DateTimeWrapperMicro> for CockroachArrowTransport<P, C> {
    fn convert(val: DateTime<Utc>) -> DateTimeWrapperMicro {
        DateTimeWrapperMicro(val)
    }
}

impl<P, C> TypeConversion<Uuid, String> for CockroachArrowTransport<P, C> {
    fn convert(val: Uuid) -> String {
        val.to_string()
    }
}

impl<P, C> TypeConversion<Decimal, f64> for CockroachArrowTransport<P, C> {
...
impl<P, C> TypeConversion<Value, String> for CockroachArrowTransport<P, C> {
